...
import { getAllServicesContractsRepo } from "../../redux/features/repositories/servicescontracts-repSlice";

const ServicesContractsRepo = () => {
    const dispatch = useDispatch();
    const servicescontractsRepository = useSelector((state) => state.servicescontractsRepo.servicescontractsRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllServicesContractsRepo({page, searchTerm})).unwrap().then((res) => {
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
            <h1 className="text-[12px] font-bold">SERVICES CONTRACTS REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                                ...
                                <th className="border border-gray-200  ">
                                    <span className="inline-flex items-center truncate-25" title="IPC FILE LABEL NUMBER">
                                        FILE LABEL NUMBER{" "}
                                        {/* <BiSort
                      size={15}
                      className={`ml-2 cursor-pointer ${sortField === "description"
                    ...
                    /> */}
                        ...
                        <tbody>
                            {servicescontractsRepository && servicescontractsRepository.length > 0 ? (
                                servicescontractsRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.contracts?.map((contract, i) => (
                                        <tr key={itemIndex + `${i}`}>
                                            ...
                                            <td className="border-y text-left ">
                                                {formatDate(contract?.serviceToBeDeliveredBeforeDate)}
                                            </td>
                                        </tr>
                                    ))
                                )
...
export default ServicesContractsRepo;
