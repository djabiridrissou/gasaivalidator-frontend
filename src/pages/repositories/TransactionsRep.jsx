...
import { getAllTransactionsRepo } from "../../redux/features/repositories/transactions-repSlice";

const TransactionsRepo = () => {
    const dispatch = useDispatch();
    const transactionsRepository = useSelector((state) => state.transactionsRepo.transactionsRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllTransactionsRepo({page, searchTerm})).unwrap().then((res) => {
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
            <h1 className="text-[12px] font-bold">TRANSACTIONS REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                        ...
                        <tbody>
                            {transactionsRepository && transactionsRepository.length > 0 ? (
                                transactionsRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.transactions?.map((transaction, i) => (
                                        <tr key={itemIndex + `${i}`}>
                                            ...
                                            <td className="border-y text-left ">
                                                {(transaction?.amountPaid)?.toLocaleString(undefined, {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2,
                                                })}
                                            </td>
