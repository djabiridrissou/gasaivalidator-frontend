...
import { getAllIpcRepo } from "../../redux/features/repositories/ipc-repSlice";

const IpcRepo = () => {
    const dispatch = useDispatch();
    const ipcRepository = useSelector((state) => state.ipcRepo.ipcRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllIpcRepo({page, searchTerm})).unwrap().then((res) => {
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
            <h1 className="text-[12px] font-bold">IPC REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                        ...
                        <tbody>
                            {ipcRepository && ipcRepository.length > 0 ? (
                                ipcRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.ipcdetails?.map((ipc, i) => (
                                        <tr key={itemIndex + `${i}`}>
                                            ...
                                            <td className="border-y text-left ">
                                                {(ipc?.ipcAmount)?.toLocaleString(undefined, {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2,
                                                })}
                                            </td>
