...
import { getAllDonorsRepo } from "../../redux/features/repositories/donors-repSlice";

const DonorsRepo = () => {
    const dispatch = useDispatch();
    const donorsRepository = useSelector((state) => state.donorsRepo.donorsRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllDonorsRepo({page, searchTerm})).unwrap().then((res) => {
            //console.log("donorsRepo", res);
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
            <h1 className="text-[12px] font-bold">DONORS REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                        ...
                        <tbody>
                            {donorsRepository && donorsRepository.length > 0 ? (
                                donorsRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.donors?.map((donor, i) => (
                                        <tr key={itemIndex + `${i}`}>
