...
import { getAllGoodscontractsRepo } from "../../redux/features/repositories/goodscontracts-repSlice";

const GoodsContractsRepo = () => {
    const dispatch = useDispatch();
    const goodscontractsRepository = useSelector((state) => state.goodscontractsRepo.goodscontractsRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllGoodscontractsRepo({page, searchTerm})).unwrap().then((res) => {
            console.log("donorsRepo", res.data);
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
             <h1 className="text-[12px] font-bold">GOODS CONTRACTS REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                                ...
                                <th className="border border-gray-200  ">
                                    <span className="inline-flex items-center truncate-25" title="UNIT PRICE">
                                    UNIT PRICE{" "}
                                        {/* <BiSort
                      size={15}
                      className={`ml-2 cursor-pointer ${sortField === "description"
                    ...
                    /> */}
                        ...
                        <tbody>
                            {goodscontractsRepository && goodscontractsRepository.length > 0 ? (
                                goodscontractsRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.contracts?.map((contract, i) => (
                                        <tr key={itemIndex + `${i}`}>
                                            ...
                                            <td className="border-y text-left ">
                                                {(contract?.unitPrice)?.toLocaleString(undefined, {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2,
                                                })}
                                            </td>
...
export default GoodsContractsRepo;
