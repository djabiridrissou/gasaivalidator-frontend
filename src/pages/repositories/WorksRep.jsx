...
import { getAllWorksRepo } from "../../redux/features/repositories/works-repSlice";

const WorksRepo = () => {
    const dispatch = useDispatch();
    const worksRepository = useSelector((state) => state.worksRepo.worksRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllWorksRepo({page, searchTerm})).unwrap().then((res) => {
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
            <h1 className="text-[12px] font-bold">WORKS COMPLETED REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                        ...
                        <tbody>
                            {worksRepository && worksRepository.length > 0 ? (
                                worksRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.works?.map((work, i) => (
                                        <tr key={itemIndex + `${i}`}>
