...
import { getAllSupplyersRepo } from "../../redux/features/repositories/supplyers-repSlice";

const SupplyersRepo = () => {
    const dispatch = useDispatch();
    const supplyersRepository = useSelector((state) => state.supplyersRepo.supplyersRepository);
    const navigate = useNavigate();
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [totalPages, setTotalPages] = useState(1);
    useEffect(() => {
        const response = dispatch(getAllSupplyersRepo({page, searchTerm})).unwrap().then((res) => {
            console.log("supplyers", res);
    ...
    return (
        <div className="container h-screen flex justify justify-start flex-col mt-1 mx-auto px-1 overflow-auto ">
            <h1 className="text-[12px] font-bold">SUPPLYERS REPOSITORY</h1>
            ...
            <div className="bg-white rounded-lg p-2 border shadow-md">
                ...
                <div className="max-h-[80vh] overflow-y-scroll">
                    ...
                    /> */}
                        ...
                        <tbody>
                            {supplyersRepository && supplyersRepository.length > 0 ? (
                                supplyersRepository?.map((item, itemIndex) => (
                                    item.gifmisProcesseds[0]?.suppliances?.map((suppliance, i) => (
                                        <tr key={itemIndex + `${i}`}>
